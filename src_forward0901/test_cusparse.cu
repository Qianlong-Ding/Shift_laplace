    ...
    for(iz=0;iz<nz;iz++)
    {
        for(ix=0;ix<nx;ix++)
        {
            //if((h_A[ix*nx+iz].x*h_A[ix*nx+iz].x)>1e-6)
            {
                ...
                //no_zero++;
                //h_A[ix*nx+iz].x=1.0;
                h_A[ix*nx+iz].x=(float) iz*nx+ix+1;
...
checkCudaErrors(cusparseCcsrsv2_analysis(handle, trans_L, m, nnz, descr_L,
    d_csrVal, d_csrRowPtr, d_csrColInd,
    info_L, policy_L, pBuffer));


checkCudaErrors(cusparseCcsrsv2_analysis(handle, trans_U, m, nnz, descr_U,
    d_csrVal, d_csrRowPtr, d_csrColInd,
    info_U, policy_U, pBuffer));

// step 5: M = L * U
checkCudaErrors(cusparseCcsrilu02(handle, m, nnz, descr_M,
...
/*checkCudaErrors(cusparseCcsrilu02(handle, m, nnz, descr_M,
    d_csrVal, d_csrRowPtr, d_csrColInd, info_M, policy_M, pBuffer));*/
//memset(h_CsrValA,        0, sizeof(cuFloatComplex)*(nx)*(nx));
//checkCudaErrors(cusparseCcsrilu0(handle,CUSPARSE_OPERATION_NON_TRANSPOSE,m,descr,
//    d_csrVal, d_csrRowPtr, d_csrColInd,infoA));
//checkCudaErrors(cudaMemcpy(h_CsrValA,d_csrVal,sizeof(cuFloatComplex)*nx*nx,cudaMemcpyDeviceToHost));

    for(ix=0;ix<nx*nx;ix++)
    {
        printf("ilu [%d]=%f\n",ix,h_CsrValA[ix].x);
    }
