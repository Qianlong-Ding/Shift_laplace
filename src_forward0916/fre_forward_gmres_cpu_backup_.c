   ...
   dblock=(int)(nfft/(npros));
    ...
    switch(op_fdmodel)
    {
        ...
        //for(ifreq=f_beg;ifreq<f_end;++ifreq)
        {
            ...
            sprintf(optname,"%s/complex_r_%d.bin",frslicepath,iifreq[ifreq]);
            optfp=fopen(optname,"w");
            ...
            sprintf(optname,"%s/complex_r2D_%d.bin",frslicepath,iifreq[ifreq]);
            optfp=fopen(optname,"w");
            ...
            {
                sprintf(optname,"%s/norm2shiftlaplace_res%d.bin",frslicepath,iifreq[ifreq]);
                optfp=fopen(optname,"w");
            ...
            {
                sprintf(optname,"%s/norm2_res%d.bin",frslicepath,iifreq[ifreq]);
                optfp=fopen(optname,"w");
                fwrite(norm2_res,sizeof(float),(nter_max+1),optfp);
                fclose(optfp);
            }
